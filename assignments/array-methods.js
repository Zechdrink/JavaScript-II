...
let largeShirts = runners.filter(run => run.shirt_size === "L");
...
// Problem 1
//The government is low on funding due to the expanding infastructure and now requires a 90% tax on donations. Use the Map method to create a new array of each taxed donation. _--_
let taxedAmount = runners.map(runners => runners.donation * .9);
console.log(taxedAmount); 

// Problem 2
// ATTENTION!!! It has been discovered that all of the runners for the Company, Skinix, are mean.
// Use the filter method to ascertain their information so that we can disqualify them.

let notMyHerd = runners.filter(run => run.company_name === 'Skinix');
console.log(notMyHerd);

// Problem 3
// Due to the Government's increased taxation, the runners are refusing to run. Use the forEach method so that we can match all donations X10 to keep them running.

let newDonations = [];
runners.forEach(function(runners){
newDonations.push(runners.donation * 10); 
});
